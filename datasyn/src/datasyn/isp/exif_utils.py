...
from datasyn.isp.exif_data_formats import exif_formats
...
def parse_exif_ifd(binary_file, offset_, endian_sign, verbose=True):
    ...
    num_entries = struct.unpack(endian_sign + "H", binary_file.read(2))[
        0
    ...
    for t in range(num_entries):
        ...
        ifd.tags.update(
            {tag_.tag_num: tag_}
        )  # supposedly, EXIF tag numbers won't repeat in the same IFD
...
def parse_exif_tag(binary_file, endian_sign, verbose=True):
    ...
    tag.num_values = struct.unpack(endian_sign + "I", binary_file.read(4))[
        0
    ...
    if tag.num_values == 1 and total_bytes < 4:
        ...
        tag.values.append(
            struct.unpack(endian_sign + exif_format.short_name, val_bytes)[0]
        )
    else:
        ...
        for k in range(tag.num_values):
            ...
            if exif_format.name == "unsigned rational":
                tag.values.append(
                    eight_bytes_to_fraction(val_bytes, endian_sign, signed=False)
                )
            elif exif_format.name == "signed rational":
                tag.values.append(
                    eight_bytes_to_fraction(val_bytes, endian_sign, signed=True)
                )
            else:
                tag.values.append(
                    struct.unpack(endian_sign + exif_format.short_name, val_bytes)[0]
