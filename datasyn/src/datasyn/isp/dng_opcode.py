...
from datasyn.isp.exif_utils import get_tag_values_from_ifds
...
def parse_opcodes(opcode_list):
    ...
    for j in range(num_opcodes):
        ...
        i += 4
        dng_spec_ver = [
            struct.unpack(endian_sign + "B", opcode_list[i + k : i + k + 1])[0]
            for k in range(4)
        ]
        i += 4
        ...
        if option_bits & 1 == 1:  # optional/unknown
            ...
            option_bits & 2 == 2
        ):  # can be skipped for "preview quality", needed for "full quality"
            pass
        else:
            pass
...
def parse_opcode_gain_map(opcode_data):
    ...
    counts = [1] * 15 + [
        0
    ]  # 0 count means variable count, depending on map_points_v and map_points_h
    # values = []
    ...
    for k in range(len(keys)):
        ...
        if counts[k] == 1:
            vals = struct.unpack(
                endian_sign + dtypes[k], opcode_data[i : i + dtype_sizes[k]]
            )[0]
            i += dtype_sizes[k]
        else:
            vals = []
    ...
    opcode_dict["map_gain_2d"] = np.reshape(
        opcode_dict["map_gain"],
