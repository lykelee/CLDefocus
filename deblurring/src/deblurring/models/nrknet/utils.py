...
# --- iter-based checkpoint helpers (training): {name}_iter{NNNNNNNN}.pth ----

def save_model_iter(model, dirname, it):
    if type(model).__name__ == name_dataparallel:
        model = model.module
    torch.save(model.state_dict(), '{}/{}_iter{:08d}.pth'.format(dirname, type(model).__name__, it))


def save_optimizer_iter(optimizer, model, dirname, it):
    if type(model).__name__ == name_dataparallel:
        model = model.module
    torch.save(optimizer.state_dict(),
               '{}/{}_iter{:08d}.pth'.format(dirname, type(optimizer).__name__ + '_' + type(model).__name__, it))


def load_model_iter(model, path, it=None, strict=True):
    """Load {name}_iter{NNNNNNNN}.pth. it=None loads the latest. Returns the iter."""
    if type(model).__name__ == name_dataparallel:
        model = model.module
    name = type(model).__name__
    if it is None:
        nums = []
        for c in glob.glob('{}/{}_iter*.pth'.format(path, name)):
            m = re.search(r'_iter(\d+)\.pth$', c)
            if m:
                nums.append(int(m.group(1)))
        if nums:
            last = max(nums)
            p = '{}/{}_iter{:08d}.pth'.format(path, name, last)
            model.load_state_dict(torch.load(p), strict=strict)
            return last
    else:
        p = '{}/{}_iter{:08d}.pth'.format(path, name, it)
        if os.path.exists(p):
            model.load_state_dict(torch.load(p), strict=strict)
            return it
        else:
